...
import { updateDetails } from "../redux/details/detailSlice";
...
export default function EditPage() {
  ...
  const [hobbies, setHobbies] = useState(item.hobbies);
  const [countryName, setCountryName] = useState(item.countryName);
  const [showCountryModal, setShowCountryModal] = useState(false);
  const [filteredCountries, setFilteredCountries] = useState(countries);
  const [capturedImage, setCapturedImage] = useState(item.image);
  const [capturedVideo, setCapturedVideo] = useState(item.video);
  const [showcaseImage, setShowcaseImage] = useState(item.image);
  const [showcaseVideo, setShowcaseVideo] = useState(item.video);
    ...
    <SafeAreaView className="flex-1">
      ...
      >
        <View className="mx-5 p-2">
          <Text className="text-center font-bold text-violet-600 text-xl mt-3">
            Edit Page
          </Text>
          <View className="my-2">
          ...
          <View>
            <TouchableOpacity
              className="p-5 bg-violet-400 rounded-md"
          ...
          <View className="my-2">
            <Text className="font-bold text-[18px] text-green-500">
              City : <Text className="text-blue-500">{cityName}</Text>
            </Text>
          </View>
          <View className="my-2">
            <TouchableOpacity
              className="p-5 bg-pink-500 rounded-md"
              onPress={() => {
                handleSubmit();
              }}
            >
